...
# from ensempy.datafiletypes.wrfout import WRFOut
from ensempy.datafiletypes.gefs import GEFS

class Ensemble:
    def __init__(self,data_info,ctrl=False,aux=None,lazy_load=True):
        ...
        Example:
                ...
                data_info = [{"model":"GFS","name":"p02",
                        "initutc": datetime.datetime(2016,3,31,18,0,0),
                        "path":"/path_to/data"},]

                E = Ensemble(data_info)
                E.get('wind10',utc=1)

        Args:
            data_info (list,tuple): a collection of dictionaries for loading
                ensemble data. Each dictionary is a member, and must contain
                the model ("GEFS","GFS","WRF","ECMWF") and the absolute
                filepath.

                Optional keys are ["name","ctrl","wrf_type",
                "np_array","utc_list","init_utc"].

                Include "name" as the ensemble-member name, and each must be
                unique; if this is not specified, each member will be named in
                sequential order of the list, with the first as "ctrl" if ctrl
                is True.

                If the model is generic WRF output, the type of simulation must
                be specified, passed as "ideal" for idealised WRF and "em_real"
                for the real-world simulations ("em_real" from WRF).

                The key {"ctrl":True} sets that member as a control member.

                The key "np_array", with a value of a 4-D numpy array, adds a
                member that skips the data format identification.
                Of course, this data cannot be lazy-loaded, and a seequence of
                times corresponding to each of the time dimension must be
                assigned to the member with "utc_list".

                For other files, if multiple times are loaded
    ...
    def load_metadata(self,):
        ...
        for mem_dict in self.data_info:
